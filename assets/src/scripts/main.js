...
const updateProgressBar = () => {
    ...
    progressBar.style.width = `${totalProgress / 2.35}%`;
...
rgbeLoader.load(
    ...
    (xhr) => {
        ...
        // console.log(hdrProgress.toFixed(0) + '% HDR loaded');
        updateProgressBar();
    },
    (error) => {
...
loader.load(
    ...
    (xhr) => {
        ...
        // console.log(modelProgress.toFixed(0) + '% loaded');
        updateProgressBar();
    },
    (error) => {
...
let targetPosition = { x: 0, y: 15, z: 0 };
let targetLookAt = { x: 0, y: 20, z: 0 };
let currentPosition = { x: camera.position.x, y: camera.position.y, z: camera.position.z };
let currentLookAt = { x: 0, y: 0, z: 0 };

const keyframes = [
    { percent: 0, position: { x: 0, y: 15, z: 0 }, lookAt: { x: 0, y: 20, z: 0 } },
    { percent: 10, position: { x: 0, y: 5, z: 10 }, lookAt: { x: 0, y: 0, z: 0 } },
    { percent: 22, position: { x: -16, y: 2, z: 10 }, lookAt: { x: 20, y: -5, z: -40 } },
    { percent: 40, position: { x: 5, y: 5, z: 10 }, lookAt: { x: 0, y: -50, z: -100 } },
    { percent: 60, position: { x: 15, y: 2, z: -5 }, lookAt: { x: -5, y: 0, z: 0 } },
    { percent: 80, position: { x: 15, y: 2, z: 5 }, lookAt: { x: -50, y: -10, z: -25 } },
    { percent: 90, position: { x: 5, y: 5, z: 5 }, lookAt: { x: -150, y: -100, z: -150 } },
